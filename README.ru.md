...
MCP-серверу: читать метаданные приложения, проверять доступность и совместимость
RasHub, получать сводку по RasGate и RAS endpoints, смотреть последние проблемы
приложения и сохранённое состояние RasGate. Для этого доступны read-only
инструменты `get_rasstudio_info`, `get_rashub_status`,
`get_infrastructure_overview`, `get_application_issues` и
`get_rasgate_status`; настройки провайдера и модели хранятся локально.
